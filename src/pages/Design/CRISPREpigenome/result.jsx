...
import { executeCRISPREpigenomeDesign, getCRISPREpigenomeResult } from "@/utils/api/apis";
...
const EpigenomeResult = () => {
    ...
    const initializeData = async () => {
      try {
        ...
        const resolution = await resolveDesignResult({
          ...
          executeRequest: executeCRISPREpigenomeDesign,
          getRequest: getCRISPREpigenomeResult,
  ...
  const cpg = resultData?.CpGIslandInfo;

  return (
    <div className="epigenome-result-container result-page-shell">
      ...
      {cpg && (
        <div className="result-section" style={{ marginTop: 12 }}>
          <Card title="CpG Island Information" bordered bodyStyle={{ padding: 12 }}>
            <div style={{ marginBottom: 8, color: cpg.found ? '#16a34a' : '#ef4444' }}>
              {cpg?.message || (cpg.found ? 'CpG island found near the locus' : 'No CpG island found')}
            </div>
            <Descriptions size="small" column={4} bordered>
              <Descriptions.Item label="Found" span={1}>{String(Boolean(cpg.found))}</Descriptions.Item>
              <Descriptions.Item label="SeqID" span={1}>{cpg.cpg_island_position?.seqid || cpg.original_position?.seqid || '-'}</Descriptions.Item>
              <Descriptions.Item label="Original start" span={1}>{cpg.original_position?.start ?? '-'}</Descriptions.Item>
              <Descriptions.Item label="Original end" span={1}>{cpg.original_position?.end ?? '-'}</Descriptions.Item>

              <Descriptions.Item label="CpG start" span={1}>{cpg.cpg_island_position?.start ?? '-'}</Descriptions.Item>
              <Descriptions.Item label="CpG end" span={1}>{cpg.cpg_island_position?.end ?? '-'}</Descriptions.Item>
              <Descriptions.Item label="GC content" span={1}>{cpg.cpg_island_position?.gc_content ?? '-'}</Descriptions.Item>
              <Descriptions.Item label="CpG ratio" span={1}>{cpg.cpg_island_position?.cpg_ratio ?? '-'}</Descriptions.Item>

              <Descriptions.Item label="Score" span={1}>{cpg.cpg_island_position?.score ?? '-'}</Descriptions.Item>
              <Descriptions.Item label="Original locus" span={3}>{
                (() => {
                  const id = cpg.original_position?.seqid;
                  const s = cpg.original_position?.start;
                  const e = cpg.original_position?.end;
                  return id && s != null && e != null ? `${id}:${s}-${e}` : '-';
                })()
              }</Descriptions.Item>
            </Descriptions>
          </Card>
...
export default EpigenomeResult;
